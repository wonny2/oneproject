...
export default function UsedItemsCommentPresenterItems(
  props: IPresenterItemsProps
) {
  ...
  const answers = data?.fetchUseditemQuestionAnswers.map(
    (i: any) => i.contents
  );
  const names = data?.fetchUseditemQuestionAnswers.map((i: any) => i.user.name);
  const createdAt = data?.fetchUseditemQuestionAnswers.map(
    (i: any) => i.createdAt
  );
      ...
      <C.ContentsWrap id={props.el._id}>
        ...
        <C.CommentOpenWrap>
          <div style={{ color: "gray" }}>
            {data?.fetchUseditemQuestionAnswers.length}개의 댓글
          </div>
          <C.CommentOpen onClick={onClickOpen}>
            {commentOpen ? "댓 글 닫 기" : "댓 글 보 기"}
          </C.CommentOpen>
        </C.CommentOpenWrap>
        {commentOpen ? (
          <C.AnswerWrap>
